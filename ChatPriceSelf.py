...
def run_entrance():
    ...
    for i in range(totalcase-1):
        print ("      正在执行caseId：%s" % (i + 1))
        matrix[i] = chat_price(host, wx_header, read_case(CaseFileName, i+1)[0], read_case(CaseFileName, i+1)[1] )
...
def chat_price(host, wx_header, mobile, price, pwd=123456):
    ...
    info = [rsp_me["mobile"], rsp_me["_id"], rsp_me["name"], rsp_me["hospital"]["name"]]

    # 获取医生可配置收入列表
    url_price_list = host + 'xxxxxxxxx'  # 填写获取医生价格列表地址
    rsp_price_list = requests.post(url=url_price_list, headers=header).json()
    price_list = [rsp_price_list["msg"]["priceList"], rsp_price_list["msg"]["currentPrice"], price]

    # 医生提交自主定价
    url_set_price = host + 'xxxxxxxxx'  # 填写提交医生价格地址
    ...
    params =dict(_id=rsp_me["_id"], bizid=rsp_me["_id"])
    rsp_wx_doc_main = requests.get(url=url_wx_doc_main, params=params, headers=wx_header).json()
    price_wx = [rsp_wx_doc_main["msg"]["priceList"]["consultationPrice"]]

    return info + price_list + price_wx
